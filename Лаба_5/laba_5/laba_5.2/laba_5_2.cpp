...
void add_contact(Contact* notebook, int& num_contacts) {
	unsigned i;
	cout << "Введите ФИО: "; cin.getline(notebook[num_contacts - 1].full_name, 50);
	cout << "Укажите адрес: "; cin.getline(notebook[num_contacts - 1].address, 100);
	cout << "Введите номер телефона: "; cin.getline(notebook[num_contacts - 1].phone, 15);
	cout << "Введите дату рождения (в формате ДД ММ ГГГГ): ";
	cin >> i; notebook[num_contacts - 1].birth_date.day = i;
	cin >> i; notebook[num_contacts - 1].birth_date.month = i;
	cin >> i; notebook[num_contacts - 1].birth_date.year = i;
	cin.ignore();
	cout << "Укажите место работы, иначе введите нет: "; cin.getline(notebook[num_contacts - 1].status.work_place, 50);
	cout << "Укажите должность: "; cin.getline(notebook[num_contacts - 1].status.position, 30);
	cout << "Укажите место учёбы, иначе введите нет: "; cin.getline(notebook[num_contacts - 1].status.study_place, 50);
...
void view_contacts(Contact* notebook, int& num_contacts) {
	if (num_contacts == 0) {
		...
		for (int i = 0; i < num_contacts; i++) {
			...
			cout << "Место учёбы: " << notebook[i].status.study_place << endl;
			cout << "Место работы: " << notebook[i].status.work_place << endl;
			cout << "Должность: " << notebook[i].status.position << endl << endl;
		}
	}
}
	...
	if (file.is_open()) {
		for (int i = 0; i < num_contacts; i++) {
			file << "ФИО: " << notebook[i].full_name << '\n'
				...
				<< "Место работы: " << notebook[i].status.work_place << '\n'
				<< "Должность: " << notebook[i].status.position << '\n'
				<< "Место учёбы: " << notebook[i].status.study_place << '\n' << '\n';
		}
